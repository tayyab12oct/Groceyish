    ...
    <div className="overflow-x-hidden">
      ...
      <div className="2xl:max-w-8xl xl:max-w-6xl max-w-6xl mx-auto md:py-20 pb-20 md:px-6 px-4 xl:px-0 grid lg:grid-cols-4 md:grid-cols-2 grid-cols-1 lg:gap-20 md:gap-10 gap-4 w-full">
        {topList.map((v, k) => {
          return (
            <div key={k}>
              <h1 className="text-black-200 lg:text-2xl text-xl font-semibold tracking-wide border-b border-black-100 pb-4 relative">
                {v.title}
              ...
              <div className="space-y-4 py-6">
                {v?.list?.map((val, i) => {
                  return (
                    <Product
                      className="mx-0 flex-row border-0 space-x-1"
                      {...val}
                      key={i}
      ...
      >
        <div className="flex items-center md:pt-4">
          <img
            src={assets.button.appStore}
            alt="AppStore"
            className="2xl:h-16 lg:h-12 md:h-10 h-8 cursor-pointer"
          />
          <img
            src={assets.button.playStore}
            alt="PlayStore"
            className="2xl:h-18 lg:h-14 md:h-11 h-9 cursor-pointer"
          />
        </div>
      </PromotionCard>
